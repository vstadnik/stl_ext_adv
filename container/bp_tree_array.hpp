...
public:
    ...
    protected:
        ...
        _iter_base ( difference_type   ind ,
                     ...
                     m_index(ind), m_ptr(pln), m_p_cont((_Ty_This*)ctr) { }
        _iter_base ( difference_type   ind ,
                     _NodeLightPtr     pln ,
                     _Ty_This *        ctr
                   ) :
...
protected:
    ...
    bp_tree_array  ( const _Pred &     pred ,
                     ...
                     bool              ord  ,
                     const _Alloc &    alr  ) ;
    bp_tree_array  ( const _Ty_This &  that ) ;
    _Ty_This &
...
protected:
    ...
    static void     _construct           ( pointer         ptr       ,
   ...
   _NodeHeavyPtr    _create_node_heavy ( _NodeLightPtr     p_light  ) ;
    ...
    void _insert_iter_seqce ( iterator  pos, _InpIter  pos_a, _InpIter  pos_b )
    {
        ...
        if ( ++pos_x == pos_b )
        {
            _insert_seqce ( pos , *pos_a ) ;
            return ;
        }

        _Ty_This    tree_cnt ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;
    ...
    void _merge_pred ( _Ty_This &  that , _PredMerge  pred )
    {
        ...
        if ( this->empty() || !pred( *b_beg , *(a_end-1) ) )
        {
        ...
        _Ty_This        b_front_seqce ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;
        _Ty_This        b_tail_seqce  ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;
        _Ty_This        middle_seqce  ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;

        if ( pred ( *b_beg , *a_beg ) )
        {
            iterator    b_low_bnd_a_beg = std::lower_bound ( b_beg , b_end , *a_beg , pred ) ;
            b_front_seqce . splice ( b_front_seqce.end() , that , b_beg , b_low_bnd_a_beg ) ;
            b_beg = that. begin() ;
            b_cur = b_beg ;
            b_end = that. end() ;
        }
        ...
        {
            iterator    a_upp_bnd_b_beg = std::lower_bound ( a_beg , a_end , *b_beg , pred ) ;
            a_front_dist = a_upp_bnd_b_beg - a_beg ;
            a_beg = a_upp_bnd_b_beg ;
            a_cur = a_upp_bnd_b_beg ;
        }

        std::vector<value_type>     vec_temp ;
    ...
    iterator        _splice_self ( iterator         pos     ,
                                   iterator         pos_a   ,
                                   iterator         pos_b   ) ;
    void            _splice_tree ( _Ty_This &       that    ) ;
    ...
    void            _reconnect   ( _Ty_This &       other   ) ;
                            ...
                            size_type      capacity    )
{
    ...
    void *              p_alloc    = malloc ( sz_block*sizeof(_NodeLight) ) ;
    _NodeLightPtr       p_lt_block = reinterpret_cast<_NodeLightPtr>(p_alloc) ;
...
BP_TREE_TY::_create_block_end ( _NodeHeavyPtr  p_parent )
{
    ...
    void *              p_alloc    = malloc ( sz_end*sizeof(_NodeLight) ) ;
    _NodeLightPtr       p_lt_block = reinterpret_cast<_NodeLightPtr>(p_alloc) ;
    _NodeLightPtr       pos_end    = p_lt_block + _offset() ;
...
void BP_TREE_TY::_delete_block_shallow ( _NodeHeavyPtr  p_parent )
{
    ...
    void *          p_alloc = reinterpret_cast<void*>(p_lt_arr) ;
    free ( p_alloc ) ;
}
...
TEMPL_DECL
void BP_TREE_TY::_erase_botm_level ( )
{
    _NodeHeavyPtr   p_tl_end = _bottom_end ( ) ;
    _NodeHeavyPtr   p_cur    = p_tl_end->p_next ;
    _NodeHeavyPtr   p_temp   = 0 ;
    ...
    _delete_block_end ( p_tl_end->_get_node_light() ) ;
    p_tl_end->p_predr->p_succr = p_tl_end->p_succr ;
    p_tl_end->p_succr->p_predr = p_tl_end->p_predr ;
    _delete_node_heavy ( p_tl_end ) ;
}
...
BP_TREE_TY::erase ( iterator  pos_a , iterator  pos_b )
{
    ...
    {
        _Ty_This        tail_b ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;
        _Ty_This        tail_a ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;
                            ...
                            iterator    pos_b )
{
    ...
    _Ty_This        this_tail    ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;
    _Ty_This        other_middle ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;
    _Ty_This        other_tail   ( m_k_comp , m_multi , m_ordered , m_allr_ty_val ) ;
                           ...
                           iterator    pos_b )
{
    ...
    _Ty_This    tree_12 ( m_k_comp, m_multi, m_ordered, m_allr_ty_val );
    _Ty_This    tree_23 ( m_k_comp, m_multi, m_ordered, m_allr_ty_val );
    _Ty_This    tree_34 ( m_k_comp, m_multi, m_ordered, m_allr_ty_val );
...
TEMPL_DECL
void BP_TREE_TY::_reconnect ( _Ty_This &  other )
{
