...
#ifndef _BP_TREE_IDX_ACC_HPP
#define _BP_TREE_IDX_ACC_HPP
...
<
    ...
    class _Alloc
 >
class bp_tree_idx_acc
{
public:
    typedef bp_tree_idx_acc < _Ty_Key , _Ty_Map , _Ty_Val , 
...
public:
    ...
    class _iter_base
    {
    friend class bp_tree_idx_acc < _Ty_Key , _Ty_Map , _Ty_Val ,
                                   _KeyOfV , _MapOfV , _Pred   , _Alloc  > ;
    ...
    bp_tree_idx_acc  ( const _Pred &     pred ,
                       bool              mul  ,
                       bool              ord  ,
                       const _Alloc &    alr  ) ;
    bp_tree_idx_acc  ( const _Ty_This &  that ) ;
    _Ty_This &
...
protected:
    ...
    mapped_type     _subsum_light    ( _NodeLightPtr    p_start ) const ;
    mapped_type     _subsum_heavy    ( _NodeHeavyPtr    p_start ) const ;
    ...
    {
        bp_tree_idx_acc <_Ty_Key, mapped_type, value_type,
...
#define BP_TREE_TY  bp_tree_idx_acc < _Ty_Key , _Ty_Map , _Ty_Val , \
                                      _KeyOfV , _MapOfV , _Pred   , _Alloc >
...
TEMPL_DECL
BP_TREE_TY::bp_tree_idx_acc ( const _Pred &   pred ,
                              bool            mul  ,
                              bool            ord  ,
                              const _Alloc &  alr  ) :
...
TEMPL_DECL
BP_TREE_TY::bp_tree_idx_acc ( const BP_TREE_TY &  that ) :
    ...
    )
{
    ...
    p_par_new->m_subsum  = subsum ;
    p_parent ->m_subsum -= subsum ;
}
...
void BP_TREE_TY::_increase_tree_height ( )
{
    ...
    _Ty_Map         subsum  = _subsum_heavy ( p_new_top_beg->p_succr ) ;
    p_new_top_beg->m_subsum = subsum ;
...
void BP_TREE_TY::_insert_b_tree ( _NodeLightPtr  p_lt_new )
{
    if ( m_size_light == 1 )
    {
        ...
        p_n_heavy->p_prev->m_subsum = _MapOfV()( p_lt_new->_elem() ) ;
    ...
    if ( p_lt_new->p_light_prev == m_p_head_light )
    {
        ...
        while ( p_beg )
        {
            ...
            ++( p_beg->m_subsz ) ;
            p_beg->m_subsum += _MapOfV()( p_lt_new->_elem() ) ;
            p_beg = p_beg->p_predr ;
        }
    }
    else
    {
        ...
        {
            ++( p_tmp->m_subsz ) ;
            p_tmp->m_subsum += _MapOfV()( p_lt_new->_elem() ) ;
            p_tmp = p_tmp->p_predr ;
        }
    }
...
BP_TREE_TY::_erase_bp_tree ( iterator  pos )
{
    ...
    if ( p_lt_erase == m_p_head_light->p_light_next )
    {
        ...
        do
        {
            ...
            --( p_beg->m_subsz ) ;
            p_beg->m_subsum -= _MapOfV()( p_lt_erase->_elem() ) ;
            p_beg = p_beg->p_predr ;
        }
        while ( p_beg ) ;
    }
    else
    {
        ...
        {
            --( p_tmp->m_subsz ) ;
            p_tmp->m_subsum -= _MapOfV()( p_lt_erase->_elem() ) ;
            p_tmp = p_tmp->p_predr ;
        }
        while ( p_tmp ) ;
    ...
    if ( p_mid && ( p_left->m_subsz + p_mid->m_subsz <= _max_degree() ) )
    {
        ...
        _merge_node_with    ( p_left , sub_size_tot , sub_sum_tot ) ;
    }
    else
    {
        _delete_node_light ( p_lt_erase ) ;
    ...
    _update_data ( p_par_break , sub_size_tot , sub_sum_tot ) ;
    ...
    )
{
    ...
    _Ty_Map         sum_mid = p_mid->m_subsum ;
    ...
    if ( p_mid->p_predr && p_mid->p_predr->p_succr==p_mid )
    {
        ...
        p_mid_par ->m_subsum -= subsum_tot  ;
        p_left_par->m_subsum += subsum_tot  ;
    }
    else
    {
        subsz_accum = 0 ;
                            ...
                            const _NodeHeavyPtr    p_parent   )
{
    ...
    size_type       move_subsz  = 0 ;
    _Ty_Map         move_subsum = _Ty_Map() ;
    _move_subsize_sum ( p_mid_new , n_move , move_subsz , move_subsum ) ;
    ...
    do
    {
        ...
        if ( n_move >= 0 )
        {
            ...
            p_tmp->m_subsum         -= move_subsum ;
            p_tmp->p_prev->m_subsum += move_subsum ;
        }
        else
        {
            p_tmp->m_subsz          += move_subsz  ;
...
void BP_TREE_TY::_balance_external( const _NodeHeavyPtr  p_parent )
{
    ...
    _Ty_Map                 sub_sum = _Ty_Map() ;
    _move_level_ext ( p_lt_mid_new , sub_sum , n_move ) ;
    ...
    do
    {
        ...
        if ( n_move >= 0 )
        {
            ...
            p_tmp->m_subsum         -= sub_sum ;
            p_tmp->p_prev->m_subsum += sub_sum ;
        }
        else
        {
            p_tmp->m_subsz          += size_type ( -n_move ) ;
    ...
    )
{
    ...
    while ( p_upd )
    {
        ...
        p_upd->m_subsum        -= sub_sum_tot  ;
        p_upd->p_prev->m_subsum+= sub_sum_tot  ;
        p_upd = _move_to_parent ( p_upd ) ;
    }
}
...
TEMPL_DECL
void BP_TREE_TY::_move_subsize_sum ( _NodeHeavyPtr &        ptr        ,
                                     const difference_type  n_move     ,
                                     size_type &            subsz_res  ,
                                     _Ty_Map &              subsum_res )
{
    ...
    if ( n_move < 0 )
    {
        ...
        {
            ptr         = ptr->p_prev   ;
            subsz_res  += ptr->m_subsz  ;
            subsum_res += ptr->m_subsum ;
        }
    }
    else
    {
...
BP_TREE_TY::_subsum_light ( _NodeLightPtr  p_start ) const
{
    _Ty_Map         sub_sum = _Ty_Map() ;
    _NodeHeavyPtr   p_par_start = p_start->p_heavy_predr ;
    while ( p_par_start == p_start->p_heavy_predr )
    {
        sub_sum += _MapOfV()( p_start->_elem() ) ;
        p_start  = p_start->p_light_next ;
    }
...
TEMPL_DECL
typename BP_TREE_TY::mapped_type
BP_TREE_TY::_subsum_heavy ( _NodeHeavyPtr  p_start ) const
{
    _Ty_Map         sub_sum = _Ty_Map() ;
    _NodeHeavyPtr   p_par_start = p_start->p_predr ;
    while ( p_par_start == p_start->p_predr )
    {
        sub_sum += p_start->m_subsum ;
...
{
    _Ty_This    contr_rev ( _Pred(), this->m_multi,
                            this->m_ordered, this->get_allocator() ) ;

    while ( ! this->empty() )
    {
        contr_rev . push_front ( this->front() ) ;
        this->pop_front ( ) ;
    }

    std::swap ( m_p_head_light , contr_rev . m_p_head_light ) ;
    std::swap ( m_p_head_heavy , contr_rev . m_p_head_heavy ) ;
    std::swap ( m_size_light   , contr_rev . m_size_light   ) ;
}
                         ...
                         mapped_type     val_in   ) const
{
    ...
    while ( p_lt_cur->p_heavy_predr->_get_node_light() != p_lt_cur )
    {
        ...
        sum_res += _MapOfV()( p_lt_cur->_elem() ) ;
        p_lt_cur = p_lt_cur->p_light_next ;
        if ( dist_cur == dist_limit )
            return sum_res ;
    }

    p_cur = p_lt_cur->p_heavy_predr ;
    while ( dist_cur < dist_limit )
    {
        while ( p_cur->p_predr && p_cur->p_predr->p_succr==p_cur )
        {
            p_cur = p_cur->p_predr ;
            if ( ( dist_cur + p_cur->m_subsz ) > dist_limit )
            {
                p_cur = p_cur->p_succr ;
                break ;
            }
        ...
        do
        {
            dist_cur += p_cur->m_subsz  ;
            sum_res  += p_cur->m_subsum ;
            p_cur     = p_cur->p_next   ;
            if ( dist_cur > dist_limit )
                break ;
        }
        while ( ( p_cur->p_predr == 0 ) ||
                ( p_cur->p_predr && p_cur->p_predr->p_succr!=p_cur ) ) ;
    ...
    _NodeHeavyPtr   p_botm_end = _bottom_end ( ) ;
    do
    {
        while ( dist_cur < dist_limit )
        {
    ...
    while ( p_cur && ( p_cur != p_botm_end ) ) ;

    p_lt_cur = p_cur->_get_node_light() ;
    while ( dist_cur < dist_limit )
    {
        ++dist_cur ;
        sum_res += _MapOfV()( p_lt_cur->_elem() ) ;
...
void BP_TREE_TY::write_shallow ( iterator             pos     ,
                                 const mapped_type &  val_new )
{
    _NodeLightPtr           p_lt_cur = pos._pointer() ;
    const mapped_type &     val_old  = _MapOfV()( p_lt_cur->_elem() ) ;
    _NodeHeavyPtr           p_parent = p_lt_cur->p_heavy_predr ;

    while ( p_parent )
    {
        p_parent->m_subsum -= val_old ;
        p_parent->m_subsum += val_new ;
        p_parent = p_parent->p_predr ;
    }

    mapped_type &   ref_map  = (mapped_type&)( _MapOfV()(p_lt_cur->_elem()) ) ;
    ref_map = val_new ;
}
